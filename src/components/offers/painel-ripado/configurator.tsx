...
export function PanelConfigurator({ product, offerSlug }: { product: CatalogProduct; offerSlug: string }) {
    ...
    <div className="grid gap-9 lg:grid-cols-2 lg:gap-14">
      <div id="product-gallery" className="lg:sticky lg:top-24 lg:self-start">
        <div className="relative aspect-[4/5] overflow-hidden rounded-lg border border-[#e0d6cb] bg-[#e8e0d7] sm:aspect-square max-sm:-mx-4 max-sm:aspect-[4/3] max-sm:rounded-none max-sm:border-x-0">
          <img src={PANEL_GALLERY[activeIndex]} alt="Painel Ripado Decorativo" className="h-full w-full object-cover" />
          <button type="button" aria-label="Imagem anterior" onClick={() => setActiveIndex((activeIndex - 1 + PANEL_GALLERY.length) % PANEL_GALLERY.length)} className="absolute left-3 top-1/2 grid h-10 w-10 -translate-y-1/2 place-items-center rounded-full bg-white/90 shadow-sm"><ChevronLeft className="h-5 w-5" /></button>
          <button type="button" aria-label="Próxima imagem" onClick={() => setActiveIndex((activeIndex + 1) % PANEL_GALLERY.length)} className="absolute right-3 top-1/2 grid h-10 w-10 -translate-y-1/2 place-items-center rounded-full bg-white/90 shadow-sm"><ChevronRight className="h-5 w-5" /></button>
          ...
          <span className="absolute bottom-3 left-3 rounded bg-[#201a17]/80 px-2.5 py-1.5 text-[10px] text-[#f2e9df]">{activeIndex + 1} / {PANEL_GALLERY.length}</span>
          <button type="button" onClick={() => window.open(PANEL_GALLERY[activeIndex], '_blank', 'noopener,noreferrer')} className="absolute bottom-3 right-3 inline-flex items-center gap-1.5 rounded bg-white/90 px-3 py-2 text-[10px] font-semibold shadow"><Maximize2 className="h-3.5 w-3.5" />Ampliar</button>
        </div>
        <div className="mt-2 flex gap-2 overflow-x-auto pb-1 max-sm:hidden">{PANEL_GALLERY.map((src, index) => <button key={src} type="button" onClick={() => setActiveIndex(index)} className={`relative h-16 w-16 shrink-0 overflow-hidden rounded-md border-2 transition ${index === activeIndex ? 'border-[#8a5a2b]' : 'border-transparent opacity-60'}`}><img src={src} alt="" className="h-full w-full object-cover" /></button>)}</div>
      </div>
      ...
      <div className="flex flex-col gap-6">
        ...
        <a href="#avaliacoes" className="flex items-center gap-2"><StarRow value={4.7} /><strong className="text-sm">4,7</strong><span className="text-sm text-[#7d6f64] underline decoration-[#d8cec2] underline-offset-4">220 avaliações</span></a>
        <div id="configurar-painel" className="flex flex-col gap-6" style={{ scrollMarginTop: 72 }}>
          <div className="border-y border-[#e6ded4] py-4">
            <div className="flex items-baseline gap-2"><strong className="font-display text-4xl font-normal">Desde 5,00 €</strong><span className="text-sm text-[#7d6f64]">por painel</span></div>
            <strong className="mt-1 block text-sm text-[#8a5a2b]">Preço promocional E-com.casa</strong>
            <p className="mt-1 text-xs text-[#7d6f64]">Preço normal no catálogo desde 20,00 €. A campanha é validada e aplicada no checkout.</p>
          </div>

          <div>
            <div className="mb-2 flex gap-2"><strong className="text-sm">Cor:</strong><span className="text-sm text-[#7d6f64]">{color === null ? 'Escolha uma opção' : selectedColor.name}</span></div>
            <div className="flex gap-2 overflow-x-auto rounded-full border border-[#e0d6cb] bg-[#fdfbf9] px-4 py-2">{PANEL_COLORS.map((option, index) => <button key={option.name} type="button" title={option.name} onClick={() => setColor(index)} className={`h-9 w-9 shrink-0 overflow-hidden rounded-full border-2 p-0.5 ${index === color ? 'border-[#8a5a2b]' : 'border-transparent'}`}><img src={option.src} alt={option.name} className="h-full w-full rounded-full object-cover" /></button>)}</div>
          </div>
          ...
          <div className="space-y-3">
            ...
            <button type="button" onClick={() => addCampaignLine(true)} className="w-full rounded-full bg-[#201a17] py-4 text-base font-semibold text-[#f7f3ef] transition hover:bg-[#8a5a2b]">Comprar agora</button>
            <button type="button" onClick={() => addCampaignLine(false)} className="w-full rounded-full border border-[#201a17] py-3.5 text-sm font-semibold transition hover:bg-[#efe7de]">Adicionar ao carrinho</button>
